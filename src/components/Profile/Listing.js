                    ...
                    <div className='col-sm-12'>
                        ...
                        <th>$ Unit Price</th> */}
                            ...
                            <td>{item.UnitPrice}</td> */}
                                            ...
                                            <td className="small" title={(item.createrAccount || '')}>{((item.createrAccount || '').slice(0, 4) + '...' + (item.createrAccount || '').slice(-4)) || ''}</td>
                                            <td>{owner._id == _id ? <button className='btn btn-sm btn-dark' onClick={(e) => { e.preventDefault(); CancelSaleListing(item._id); }}>Cancel</button> : null}</td>
                                        </tr>
                                    )
                                })}
                            </tbody>
